...
void insertEmployee(MYSQL* conn, struct Employee emp)
{
		if (findEmployee(conn, emp.employeeNumber, &emp) == 1) {
			...
			string insertEmployee = "INSERT  INTO employees(employeeNumber,lastName,firstName,extension,email,officeCode,reportsTo,jobTitle)  VALUES ('"  + to_string(emp.employeeNumber)+"', '"+ emp.lastName +"', '"+ emp.firstName +"', '"+emp.extension+"', '"+ emp.email + "', 1, 1002, '"+emp.jobTitle+"')";

			const char* command = insertEmployee.c_str();

			int exequery;
			exequery = mysql_query(conn, command);
...
void updateEmployee(MYSQL* conn, int employeeNumber)
{
	...
	checkingResult = findEmployee(conn, employeeNumber);
	if (checkingResult == 1) {
		string ext;
		cout << "Enter the new phone extension: ";
		cin >> ext;
		
		string updateEmployeeExtension = "UPDATE employees  SET extension = '" + ext + "'WHERE employeeNumber=" + to_string(employeeNumber);
	
		const char* command = updateEmployeeExtension.c_str();
...
void deleteEmployee(MYSQL* conn, int employeeNumber)
{
	Employee emp = { 0 };
	int checkingResult = 0;
	checkingResult = findEmployee(conn, employeeNumber, &emp);
	if (checkingResult == 1) {
		string removeEmployee = "DELETE FROM employees WHERE employeeNumber =" + to_string(employeeNumber)+ ";";
		

		const char* command = removeEmployee.c_str();
