...
int findEmployee(MYSQL* conn, int employeeNumber,struct Employee* emp)
{
	...
	if (conn != nullptr) {
		string command = "SELECT employeeNumber, lastName, firstName, email, phone, extension,managerName, jobTitle, city FROM employees  INNER JOIN employees m ON m.employeeNumber = employees.reportsTo INNER JOIN offices ON offices.officeCode = employees.officeCode WHERE employees.employeeNumber = ";
...
{
	string allEmployees = "SELECT employeeNumber, CONCAT(e.firstName,' ', e.lastName) employeeName, email, phone,extension, CONCAT(m.firstName, ' ', m.lastName) managerName FROM employees e INNER JOIN employees m ON manager.employeeNumber = employees.reportsTo INNER JOIN offices ON offices.officeCode = employees.officeCode";
